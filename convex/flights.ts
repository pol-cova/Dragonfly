...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("flights", {
      ...
      encryptedCompletionCredential: generated.completionCredential,
...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    if (flight.status === "active" && now > flight.expiresAt) {
      return {
...
export const complete = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    return {
      claimReady: true,
      completionCredential: flight.encryptedCompletionCredential,
