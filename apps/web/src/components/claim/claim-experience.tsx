...
export function ClaimExperience() {
  ...
  async function handleClaim() {
    ...
    try {
      ...
      const creds = await completeFlight({ flightId: params.flightId });

      setStatus("proving");
      const proof = await deriveClaimProof({
        dropId: creds.dropId,
      ...
      const midnight = await submitMidnightClaim(
        {
          dropId: creds.dropId,
          flightId: creds.flightId,
          alias: creds.alias,
      ...
      if (!midnight.verified) {
        throw new Error("Private proof verification failed");
      }

      const result = await recordClaim({
        flightId: params.flightId,
        badgeSeed: proof.badgeSeed,
        alias: creds.alias,
      });
      ...
      setMessage(
        result.duplicate
          ...
          : `Verified via ${midnight.mode}. Credential never published.`,
      );
    } catch (e) {
      setStatus("error");
