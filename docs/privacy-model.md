...
Convex is the trusted game referee for stage validation. It never exposes credentials via queries. The `complete` action returns the credential once over the authenticated session.

## Limitations

This MVP is not a fully trustless CTF engine. Stage correctness is enforced by Convex. Midnight enforces private credential claims and duplicate prevention at the ownership layer.
