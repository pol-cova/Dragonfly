...
function operatorSecretBytes(): Uint8Array {
  const fromEnv = process.env.DRAGONFLY_OPERATOR_SECRET;
  const bytes = new Uint8Array(32);
  if (fromEnv) {
    const normalized = fromEnv.startsWith('0x') ? fromEnv.slice(2) : fromEnv;
    if (normalized.length !== 64) {
      throw new Error('DRAGONFLY_OPERATOR_SECRET must be 32 bytes (64 hex chars)');
    }
    for (let i = 0; i < 32; i++) {
